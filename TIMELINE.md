...
### Claude at 50/Day (The Baseline Scenario)

At 50 conversions per day with the moderate estimate of 6,000 file-pairs:
...
- **Note**: 50/day falls here. A single Claude instance running `loop.sh`
  continuously achieves this if average file complexity is moderate.
...
| Phase 5 (Validate) | 0–1 | Validation is mostly local tooling |
...
> **Note**: Costs include the monthly subscription fee ($10 Pro, $39 Pro+) plus
> overage charges at $0.04/request above the included allowance (300 for Pro,
> 1,500 for Pro+). Costs assume a 1x request multiplier (base model). Using
> Claude Opus (10x multiplier) or GPT-4.5 (50x multiplier) dramatically
> increases costs. A single 50/day pipeline on Claude Opus would cost
> ~$7,180/month in premium requests alone.
...
- **~6,000 C/C++ shim headers** (`*_shim.h`), providing backward-compatible
  wrappers
- **~6,000 gutted original files** (`.h` and `.cpp`/`.c`), each containing
  only a single `#include` redirect
- **~6,000 MEMORIES files** documenting discovered memory safety issues in the
  original C/C++ code

The C++ shim layer remains as a thin compatibility layer. Callers are unchanged.
The actual implementation lives in Rust.

### What "Done" Does NOT Look Like

- It does **not** mean "zero C++ in Firefox." The shim layer is still C++.
