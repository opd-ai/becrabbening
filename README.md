...
A systematic methodology for incrementally replacing C++ in Firefox with Rust — one file-pair at a time — while maintaining full backward-compatibility and avoiding merge conflicts.

---

## What Is This?

The Becrabbening is a conflict-free workflow for converting Firefox C++ code to Rust. The key insight is a **three-layer sandwich** architecture that lets you swap out the C++ internals of any file-pair without touching a single existing caller:

```
┌──────────────────────────────────────────┐
│  Existing C++ callers (unchanged)        │
├──────────────────────────────────────────┤
│  Layer 3: C++ Shim (backward compat)     │
├──────────────────────────────────────────┤
...
Each iteration converts exactly **one file-pair** (`.cpp` + `.h`, or just `.h`) through 7 phases (0–6). The workflow is designed to avoid merge conflicts at all costs by using additive-only changes for new files and minimal atomic edits to existing files.
...
|---|---|
| **One pair at a time** | Each PR converts exactly one `.cpp`+`.h` (or `.h`-only) file-pair. Never bundle multiple conversions. |
...
| **Three-layer sandwich** | Rust → C FFI → C++ Shim. Callers never know anything changed. |
